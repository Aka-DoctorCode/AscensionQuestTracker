-------------------------------------------------------------------------------
-- Project: AscensionQuestTracker
-- Author: Aka-DoctorCode
-- File: WorldQuestProvider.lua
-------------------------------------------------------------------------------
---@diagnostic disable: undefined-global, undefined-field, inject-field

-------------------------------------------------------------------------------
-- 1. INITIALIZATION
-------------------------------------------------------------------------------
local _, addonTable = ...

local worldQuestData = {}
addonTable.dataEngine:registerModule("WorldQuestData", worldQuestData)

worldQuestData.activeQuests = {}

local function getZoneMapID(mapID)
    local currentMapID = mapID
    local mapInfo = C_Map.GetMapInfo(currentMapID)
    
    while mapInfo and mapInfo.mapType and mapInfo.mapType > Enum.UIMapType.Zone do
        currentMapID = mapInfo.parentMapID
        mapInfo = C_Map.GetMapInfo(currentMapID)
    end
    
    return currentMapID or mapID
end

local function fetchMapTasks(uiMapID)
    if C_TaskQuest and C_TaskQuest.GetQuestsOnMap then
        return C_TaskQuest.GetQuestsOnMap(uiMapID) or {}
    end
    return {}
end

local function isQuestTracked(qID)
    if not qID then return false end
    if C_QuestLog.IsOnQuest and C_QuestLog.IsOnQuest(qID) then return true end
    if C_QuestLog.GetQuestWatchType and C_QuestLog.GetQuestWatchType(qID) ~= nil then return true end
    if C_QuestLog.IsQuestWatched and C_QuestLog.IsQuestWatched(qID) then return true end
    return false
end

local function isWorldQuest(questID)
    if C_QuestLog.IsWorldQuest then
        return C_QuestLog.IsWorldQuest(questID)
    end
    return C_TaskQuest.IsActive and C_TaskQuest.IsActive(questID)
end

local function getQuestTitle(questID)
    if C_TaskQuest.GetQuestInfoByQuestID then
        return C_TaskQuest.GetQuestInfoByQuestID(questID)
    end
    local info = C_QuestLog.GetInfo(questID)
    return info and info.title
end

local function getTimeLeft(questID)
    if C_TaskQuest.GetQuestTimeLeftMinutes then
        return C_TaskQuest.GetQuestTimeLeftMinutes(questID)
    end
    return nil
end

function worldQuestData:init()
    self.eventFrame = CreateFrame("Frame")
    self.eventFrame:RegisterEvent("ZONE_CHANGED_NEW_AREA")
    self.eventFrame:RegisterEvent("ZONE_CHANGED")
    self.eventFrame:RegisterEvent("QUEST_ACCEPTED")
    self.eventFrame:RegisterEvent("QUEST_REMOVED")
    self.eventFrame:RegisterEvent("QUEST_WATCH_LIST_CHANGED")
    self.eventFrame:SetScript("OnEvent", function()
        if addonTable.dataEngine then
            addonTable.dataEngine:queueUpdate()
        end
    end)
end

function worldQuestData:update()
    if self.tickers then
        for _, ticker in pairs(self.tickers) do
            ticker:Cancel()
        end
        wipe(self.tickers)
    end
    wipe(self.activeQuests)

    local uiMapID = C_Map.GetBestMapForUnit("player")
    if not uiMapID then return end

    local zoneMapID = getZoneMapID(uiMapID)
    local processedQuests = {}

    local taskPOIs = fetchMapTasks(zoneMapID)
    for _, poi in ipairs(taskPOIs) do
        local qID = poi.questId or poi.questID
        local tracked = poi.inProgress or isQuestTracked(qID)
        if qID and tracked then
            self:parseWorldQuest(qID)
            processedQuests[qID] = true
        end
    end

    if C_QuestLog.GetNumWorldQuestWatches then
        for i = 1, C_QuestLog.GetNumWorldQuestWatches() do
            local wqID = C_QuestLog.GetQuestIDForWorldQuestWatchIndex(i)
            if wqID and not processedQuests[wqID] then
                self:parseWorldQuest(wqID)
                processedQuests[wqID] = true
            end
        end
    end

    if C_BountyInfo and C_BountyInfo.GetBountiesForMapID then
        local bounties = C_BountyInfo.GetBountiesForMapID(zoneMapID)
        if bounties then
            for _, bounty in ipairs(bounties) do
                local qID = type(bounty) == "table" and (bounty.bountyQuestID or bounty.questID) or bounty
                if qID and not processedQuests[qID] then
                    self:parseWorldQuest(qID)
                    processedQuests[qID] = true
                end
            end
        end
    end
end

function worldQuestData:parseWorldQuest(questID)
    local isWQ = isWorldQuest(questID)
    if not isWQ then return end -- Bonus Objectives are handled in BonusObjectiveProvider

    local taskName = getQuestTitle(questID)
    if not taskName then
        taskName = "Loading Objective..."
    end
    
    local minutesLeft = isWQ and getTimeLeft(questID) or nil

    if C_QuestLog.GetQuestTagInfo then
        local tagInfo = C_QuestLog.GetQuestTagInfo(questID)
        if tagInfo and tagInfo.tagName and tagInfo.tagName ~= "" and tagInfo.tagName ~= "World Quest" then
            taskName = "[" .. tagInfo.tagName .. "] " .. taskName
        end
    end
        
        -- Pull objectives
        local objectives = {}
        if C_QuestLog.GetQuestObjectives then
            local objs = C_QuestLog.GetQuestObjectives(questID)
            if objs then
                for _, obj in ipairs(objs) do
                    table.insert(objectives, {
                        text = obj.text,
                        numFulfilled = obj.numFulfilled,
                        numRequired = obj.numRequired,
                        finished = obj.finished
                    })
                end
            end
        end

        table.insert(self.activeQuests, {
            id = questID,
            title = taskName,
            minutesLeft = minutesLeft,
            objectives = objectives,
            isWorldQuest = isWQ
        })
        if isWQ then
            self:startTicker(questID)
        end
end

function worldQuestData:startTicker(questID)
    if not self.tickers then self.tickers = {} end
    if self.tickers[questID] then return end

    self.tickers[questID] = C_Timer.NewTicker(60, function()
        local minutesLeft = getTimeLeft(questID)
        if not minutesLeft or minutesLeft <= 0 then
            if self.tickers[questID] then
                self.tickers[questID]:Cancel()
                self.tickers[questID] = nil
            end
        end
    end)
end
